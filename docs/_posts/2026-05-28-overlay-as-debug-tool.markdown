...
Worth repeating the most impotant feature of this modifer: adding an `overlay` modifier will never change the layout space the owner occupies. This means it can always be added as a **non-destructive** layout operation to add visual cues helping figure intricate layout problems.
